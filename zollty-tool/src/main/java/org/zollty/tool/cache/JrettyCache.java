...
public class JrettyCache {
    ...
    private static final long DEFUALT_EXPIRE_TIME = 20 * 1000;// 默认过期时间 20秒
    ...
    public JrettyCache(int initialCapacity, long expireTime) {
        ...
        cacheEntry = new WeakHashMap<>(capacity);
     ...
     * 判断容器内是否有这个item（过期的先删掉），如果没有，返回true；如果有则返回false。
     */
    public boolean putIfAbsent(Object key, Object value, long expireTimeMs) {
        if (get(key) != null) {
            return false;
        }
    ...
    protected <T> T getData(Object key) {
