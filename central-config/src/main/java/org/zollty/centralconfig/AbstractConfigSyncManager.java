 ...
 * @since 2016-5-16
 */
public class AbstractConfigSyncManager {
    ...
    protected String defaultConfigFilename = null;
    ...
    public void setLevels(String path) {
        setLevelsMap(parsePath(path));
    }

    private Map<Integer, String> parsePath(String path) {
        Map<Integer, String> paths = new HashMap<Integer, String>();
        String[] pathArray = path.split("/");
        int l = 1;
        for(int i = 0; i < pathArray.length; i++) {
            if ("".equals(trimVal(pathArray[i]))) {
                continue;
            } else {
                paths.put(l++, pathArray[i]);
            }
        }
        return paths;
    }
    ...
    protected void checkClientConfig() {
        if(defaultConfigFilename == null || defaultConfigFilename.isEmpty()) {
            defaultConfigFilename  = "config.properties";
        }
        String defaultZookeeperURL    = "localhost:2181";
        String defaultPropsPathPrefix = "/config/list";
        String defaultUsingPathPrefix = "/config/using";
        String defaultPriorityVal     = "3";
        
        // 检测基本配置
        propsPathPrefix = localPropsService.getStringDefValue("configcenter.zookeeper.props.prefix", propsPathPrefix, defaultPropsPathPrefix);
        usingPathPrefix = localPropsService.getStringDefValue("configcenter.zookeeper.using.prefix", usingPathPrefix, defaultUsingPathPrefix);
        zookeeperUrl    = localPropsService.getStringDefValue("configcenter.zookeeper.url",          zookeeperUrl   , defaultZookeeperURL   );
//        configFilename  = localPropsService.getStringDefValue("configcenter.config.filename",        configFilename , defaultConfigFilename );
        String tmp = localPropsService.getStringDefValue("configcenter.config.filename", "", "");
        if (tmp.length() > 0) {
        ...
        if (null == priority) {
            priority = Integer.parseInt(localPropsService.getStringDefValue("configcenter.config.priority", "", defaultPriorityVal));
        }
        
        // 检测路径信息
        Map<Integer, String> paths = parsePath(localPropsService.getStringDefValue("configcenter.levels", "", ""));
        Properties isversions = parseIsVersions(localPropsService.getStringDefValue("configcenter.levels.isversions", "", ""));
        
        for(int n=1; n<=9; n++) {
            if(trimVal(getLevel(n)).isEmpty()) {
                setLevel(n, localPropsService.getStringDefValue("configcenter.level."+n, "", paths.get(n)));
                setIsVersion(n, Boolean.parseBoolean(localPropsService.getStringDefValue("configcenter.level."+n+".isversion", "", isversions.getProperty(String.valueOf(n), "false"))));
            }
        }
        
        // 候选属性名
        if(trimVal(getLevel(1)).isEmpty()) setSystem   (localPropsService.getStringDefValue("configcenter.level.system"   , getSystem()   , ""));
        if(trimVal(getLevel(2)).isEmpty()) setServer   (localPropsService.getStringDefValue("configcenter.level.server"   , getServer()   , ""));
        if(trimVal(getLevel(3)).isEmpty()) setVersion  (localPropsService.getStringDefValue("configcenter.level.version"  , getVersion()  , ""));
        if(trimVal(getLevel(4)).isEmpty()) setClusterID(localPropsService.getStringDefValue("configcenter.level.clusterid", getClusterID(), ""));
        if(trimVal(getLevel(5)).isEmpty()) setSlaveID  (localPropsService.getStringDefValue("configcenter.level.slaveid"  , getSlaveID()  , ""));
    }

    
    protected void detectLocalIp() {
        List<String> zk = new ArrayList<String>(Arrays.asList(zookeeperUrl.split(",")[0].split("\\:")));
        if(zk.size() < 2) zk.add("2181");
    ...
    protected void checkVersion() throws Exception {
        ...
        while (iterator.hasNext()) {
            level = iterator.next();
            if(levels.get(level) == null || levels.get(level).isEmpty()) continue;
            parentPath = selfPath;
            ...
            if (getIsVersion(level)) {
                ...
                if (null == stat) {
                    ...
                    List<String> pathArray = client.getChildren().forPath(
                            parentPath);
                    ...
                    for (String path : pathArray) {
                        ...
                        if (null == minVersion
                                || compareVersion(path, minVersion) < 0) {
                            minVersion = path;
                        }
    ...
    private static final Pattern regexVersion = Pattern
            .compile("\\d+(\\.\\d+)*");
    ...
    private int compareVersion(String path, String version) {
        ...
        if (null == version) {
            ...
            int length = pathArray.length < versionArray.length ? pathArray.length
                    : versionArray.length;
            // 根据长度最短的数组比较值,若根据最短长度未比较出结果
            // 则数组长度大的值大于长度小的值
            for (int i = 0; i < length; i++) {
                if (Integer.parseInt(pathArray[i]) > Integer
                        .parseInt(versionArray[i])) {
                    returnVal = 1;
                    break;
                }
                if (Integer.parseInt(pathArray[i]) == Integer
